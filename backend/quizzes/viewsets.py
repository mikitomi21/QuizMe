...
class QuizViewSet(viewsets.ModelViewSet):
    ...
    def post_question(self, request, pk=None):
        ...
        if question_serializer.is_valid():
            ...
            return Response({"message": f"Question is added to the quiz nr.{quiz.id}"}, status=status.HTTP_201_CREATED)
...
class QuestionViewSet(viewsets.ModelViewSet):
    ...
    def post_answer(self, request, pk=None):
        ...
        if answer_serializer.is_valid():
            ...
            return Response({"message": f"Answer is added to the question nr.{question.id}"}, status=status.HTTP_201_CREATED)
